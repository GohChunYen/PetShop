...
async def find_pets_by_owner_name(db: db_dependency, owner_name: str = Query(...)):
    owner_models = db.query(Owners).filter(
        func.concat(Owners.first_name, " ", Owners.last_name).ilike(f"%{owner_name}%")).all()
...
@router.delete("/owners/{owner_id}/pets/{pet_id}", status_code=status.HTTP_204_NO_CONTENT)
